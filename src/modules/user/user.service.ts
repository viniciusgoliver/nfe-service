...
export class UserService {
  ...
  async create(createDto: UserCreateDTO): Promise<UserReturnUserDTO> {
    try {
      ...
      if (error.code === 'P2002') {
        ...
        throw new UnprocessableEntityException('Erro ao realizar o cadastro do usuário')
      }
    }
  }
  ...
  async confirmationEmail(confirmationToken: string): Promise<void> {
    await this.userRepository.confirmationEmail(confirmationToken)
  }
  ...
  async resetPassword(token, resetPasswordDto: AuthResetPasswordDTO): Promise<void> {
    await this.userRepository.resetPassword(token, resetPasswordDto)
  }

  async changePassword(id: number, changePasswordDto: AuthResetPasswordDTO): Promise<void> {
    await this.userExists(id)
    await this.userRepository.changePassword(id, changePasswordDto)
  }
}
