...
import { type UserReturnUserinfoDTO } from '../user/dtos/return-userinfo.dto'
...
export class AuthService {
  ...
  async saveUser(createDto: UserCreateDTO): Promise<UserReturnUserDTO> {
    try {
      ...
      if (error.code === 'P2002') {
        ...
        throw new UnprocessableEntityException(`Erro ao realizar o cadastro do usuário: ${error}`)
      }
    }
  }
  ...
  async confirmationEmail(confirmationToken: string): Promise<void> {
    await this.userService.confirmationEmail(confirmationToken)
  }
  ...
  async sendRecoverPasswordJob(email: string): Promise<void> {
    ...
    await this.queueProducerService.sendRecoverPasswordJob(email)
  }
  ...
  async resetPassword(token: string, resetPasswordDto: AuthResetPasswordDTO): Promise<void> {
  ...
  async logout(user: UserReturnUserinfoDTO): Promise<void> {
    await this.authRepository.deleteSessionUserByIdUser(user.id)
  }
}
